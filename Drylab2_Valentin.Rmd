...
regions <- DotPlot(seurat, cluster.idents = TRUE, features = c("FOXG1","EMX1", "DLX2"))
non_telen <- DotPlot(seurat, cluster.idents = TRUE, features = c("FOXG1","DCX", "SOX2"))
ventral_telen <- DotPlot(seurat, cluster.idents = TRUE, features = c("DLX2", "GAD1", "DCX", "SOX2"))
dorsal_telen <- DotPlot(seurat, cluster.idents = TRUE, features = c("EMX1", "DCX", "SOX2"))

a <- FeaturePlot(seurat, reduction = "umap_css", features = c("FOXG1","EMX1", "DLX2", "GAD1", "DCX", "SOX2"))
...
For annotation_1: (NOT CORRECT)
ventral telen clusters: 3, 5, 16
doral telen clusters: 6, 14, 15, 21
telencephalon_mixed: 4, 8, 12, 18
non-telencephalon clusters: 0, 1, 2, 7, 9, 10, 11, 13, 17, 19, 20
annotation_1 --> not correct
```{r}
seurat@meta.data$region <- factor(setNames(c("non-telen.", #0
                                   "non-telen.", #1
                                   "non-telen.", #2
                                   "ventral telen.", #3
                                   "mixed", #4
...
plot2 <- FeaturePlot(seurat, c("FOXG1","EMX1", "DLX2", "GAD1", "DCX", "SOX2", "MKI67"),
                     reduction = "umap_css", order=T) & NoAxes() & NoLegend()
plot1 | plot2
```
...
telencephalon: 3, 4, 5, 6, 8, 12, 14, 15, 16, 18, 21

ventral telen clusters: 3, 4, 5, 8, 12, 16, 18
progenitor ventral telen clusters: 4, 8, 12
differentiated ventral telen clusters: 3, 5
mixed ventral telen clusters: 16, 18

dorsal telen clusters: 6, 14, 15, 21
progenitor dorsal telen clusters: 6, 15
differentiated ventral telen clusters: 14
mixed dorsal telen clusters: 21


non-telencephalon clusters: 0, 1, 2, 7, 9, 10, 11, 13, 17, 19, 20
progenitors non-telencephalon clusters: 1, 7, 9, 11, 20
differentiated non-telencephalon clusters: 0, 2, 10, 17, 19
mixed non-telencephalon clusters: 13
...
seurat@meta.data$region <- factor(setNames(c("non-telen.", #0
                                   ...
                                   "dorsal telen."), #21
                                 levels(seurat@active.ident))[seurat@active.ident],
                        levels=c("non-telen.","ventral telen.","dorsal telen."))
plot1 <- DimPlot(seurat, reduction = "umap_css", group.by = "region") + scale_color_manual(values = custom_colors) & NoAxes()
plot2 <- FeaturePlot(seurat, c("FOXG1","EMX1", "DLX2", "GAD1", "DCX", "SOX2", "MKI67"),
                     reduction = "umap_css", order=T) & NoAxes() & NoLegend()
plot1 | plot2
```
Saving seurat object with regions

```{r}
saveRDS(seurat, file="regions.rds")
```

```{r}
seurat <- readRDS("regions.rds")
```
...
```{r}
custom_colors <- c(
  "progenitor dorsal telen." = "#ffb3b3", 
  "mixed dorsal telen." = "#ff6666",
  "diff. dorsal telen." = "#cc0000",
  "progentior ventral telen." = "#90ee90", 
  "mixed ventral telen." = "#3cb371", 
  "diff. ventral telen." = "#006400",     
  "progenitor non-telen." = "#add8e6", 
  "mixed non-telen." = "#6495ed",   
  "diff. non-telen." = "#00008b"         
)
seurat@meta.data$subregion <- factor(setNames(c("diff. non-telen.", #0
                                   "progenitor non-telen.", #1
                                   "diff. non-telen.", #2
                                   "diff. ventral telen.", #3
                                   "progentior ventral telen.", #4
                                   "diff. ventral telen.", #5
                                   "progenitor dorsal telen.", #6
                                   "progenitor non-telen.", #7
                                   "progentior ventral telen.", #8
                                   "progenitor non-telen.", #9
                                   "diff. non-telen.", #10
                                   "progenitor non-telen.", #11
                                   "progentior ventral telen.", #12
                                   "mixed non-telen.", #13
                                   "diff. dorsal telen.", #14
                                   "progenitor dorsal telen.", #15
                                   "mixed ventral telen.", #16
                                   "diff. non-telen.", #17
                                   "mixed ventral telen.", #18
                                   "diff. non-telen.", #19
                                   "progenitor non-telen.", #20
                                   "mixed dorsal telen."), #21
                                 levels(seurat@active.ident))[seurat@active.ident],
                        levels=c("progenitor non-telen.", "mixed non-telen.", "diff. non-telen.", "progentior ventral telen.", "mixed ventral telen.", "diff. ventral telen.", "progenitor dorsal telen.", "mixed dorsal telen.", "diff. dorsal telen."))
plot1 <- DimPlot(seurat, reduction = "umap_css", group.by = "subregion")  + scale_color_manual(values = custom_colors) & NoAxes()
plot2 <- FeaturePlot(seurat, c("FOXG1","EMX1", "DLX2", "GAD1", "DCX", "SOX2", "MKI67"),
                     reduction = "umap_css", order=T) & NoAxes() & NoLegend()
plot1 | plot2
```
Saving seurat object with subregions

```{r}
saveRDS(seurat, file="subregions.rds")
```

```{r}
seurat <- readRDS("subregions.rds")
```
...
barplot(freq, col=c("#6495ed","#3cb371","#ff6666"),
        border=NA, las=2, ylab="Frequency", cex.names = 0.8)
barplot(prop, col=c("#6495ed","#3cb371","#ff6666"),
        border=NA, las=2, ylab="Proportion", cex.names = 0.8)
plot.new()
legend("left", fill=c("#6495ed","#3cb371","#ff6666"), legend=rownames(freq), bty="n")
```
...
(with sub-regions)
```{r}
freq <- table(seurat$subregion, seurat$organoid)
prop <- apply(freq,2,function(x) x/sum(x))

layout(matrix(1:3,nrow=1)); par(mar=c(8,5,1,1))
barplot(freq, col=c("#add8e6","#6495ed","#00008b","#90ee90","#3cb371","#006400","#ffb3b3","#ff6666","#cc0000"),
        border=NA, las=2, ylab="Frequency", cex.names = 0.8)
barplot(prop, col=c("#add8e6","#6495ed","#00008b","#90ee90","#3cb371","#006400","#ffb3b3","#ff6666","#cc0000"),
        border=NA, las=2, ylab="Proportion", cex.names = 0.8)
plot.new()
legend("left", fill=c("#add8e6","#6495ed","#00008b","#90ee90","#3cb371","#006400","#ffb3b3","#ff6666","#cc0000"), legend=rownames(freq), bty="n")
```


Everything here is continued with "regions", not "sub-regions" --> would need to be changed in syntax
...
11. Problems with those statistics
...
Solution 1: Check visually or have stringent requirements

--> Example all the samples of one condition have to be higher than the samples of the other condition.
IN this case all samples of KO must have more cells in ventral telencephalon than all the samples of WT condition

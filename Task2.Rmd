...
# filter cells: 200-60k genes, <10% mt genes, 200-60k RNA counts
...
seurat <- ScaleData(seurat, vars.to.regress = c("G2M.Score","S.Score")) %>% # regress out cell cycle scores
  ...
  RunUMAP(dims = 1:20, min.dist = 0.5, spread = 0.5)
...
seurat@meta.data$region <- factor(setNames(c("ventral telen.",
                        ...
                        levels=c("ventral telen.","non-telen.","dorsal telen.","mixed"))
...
# this is better than Zhiso's code
EnhancedVolcano(DE_wilcoxauc_ventral, x = "logFC", y = "padj", lab = DE_wilcoxauc_ventral$DEG, title = "Ventral telen.", pCutoff = 0.01, FCcutoff = log(1.1), xlim = c(-1,1))
...
# change aspect ratio of volcano plot
EnhancedVolcano(DE_deseq2_ventral, x = "log2FoldChange", y = "padj", lab = DE_deseq2_ventral$gene, title = "Ventral telen.", pCutoff = 0.01, FCcutoff = 1.1)
...
plot1 <- EnhancedVolcano(DE_aov, lab = rownames(DE_aov), x = 'condKO', y = 'padj', title = 'aov', pCutoff = 0.01, FCcutoff = 0.1, xlim = c(-1,1))
plot2 <- EnhancedVolcano(DE_aov_bin, lab = rownames(DE_aov_bin), x = 'condKO', y = 'padj', xlim = c(-20, 20), title = 'aov_bin', pCutoff = 0.01, FCcutoff = 1.1)
plot1 | plot2
```
...
enrichment_results <- enrichr(DE_deseq2_ventral$gene, databases = databases)
...
GSEA <- function(gene_list) {
  ...
  genes_entrez <- bitr(gene_list$gene, 
                       fromType = "SYMBOL", 
                       toType = "ENTREZID", 
                       OrgDb = org.Hs.eg.db)
  ...
  gene_list <- gene_list %>%
    inner_join(genes_entrez, by = c("gene" = "SYMBOL"))
...
test <- GSEA(DE_deseq2_ventral)

```

## visualize the results
```{r}
ggplot(test, aes(x = reorder(Description, NES), y = NES, size = setSize, color = p.adjust)) +
